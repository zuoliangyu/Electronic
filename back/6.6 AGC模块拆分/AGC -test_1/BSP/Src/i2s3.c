...
static void I2S3_LowLevel_Init(void)
{
	...
	GPIO_PinAFConfig(GPIOC, GPIO_PinSource11, GPIO_AF7_SPI3); // I2S3ext_SD是AF7!!!
}
...
u8 I2S3_SampleRate_Set(u32 samplerate)
{
	...
	for (i = 0; i < (sizeof(I2S3_PSC_TBL) / 10); i++) // 看看改采样率是否可以支持
	{
		if (samplerate == I2S3_PSC_TBL[i][0])
			break;
	}

	RCC_PLLI2SCmd(DISABLE); // 先关闭PLLI2S
	if (i == (sizeof(I2S3_PSC_TBL) / 10))
		return 1;																													// 搜遍了也找不到
	RCC_PLLI2SConfig((u32)I2S3_PSC_TBL[i][1], (u32)I2S3_PSC_TBL[i][2]); // 设置I2SxCLK的频率(x=2)  设置PLLI2SN PLLI2SR

	RCC->CR |= 1 << 26; // 开启I2S时钟
	while ((RCC->CR & 1 << 27) == 0)
		;																	// 等待I2S时钟开启成功.
	tempreg = I2S3_PSC_TBL[i][3] << 0;	// 设置I2SDIV
	tempreg |= I2S3_PSC_TBL[i][3] << 8; // 设置ODD位
...
{
	if (DMA_GetITStatus(DMA1_Stream5, DMA_IT_TCIF4) == SET) ////DMA1_Stream4,传输完成标志
	{
